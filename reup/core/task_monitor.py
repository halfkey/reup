...
class TaskMonitor(BaseMonitor):
    def __init__(self, notebook, urls: List[str], parent):
        ...
        self.scanning_chars = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏']
        self.tab_chars = ['⠋', '⠙', '⠚', '⠞', '⠖', '⠦', '⠴', '⠲', '⠳', '⠓']  # Smoother animation
    ...
    def create_found_products_section(self):
        ...
        frame = ttk.LabelFrame(self, text=" Found In Stock ", style='Custom.TLabelframe')
        frame.pack(fill=tk.X, padx=10, pady=5)
    ...
    def stop_monitoring(self):
        ...
        base_text = current_text.split(' ')[-1]  # Get text after any existing indicators
    ...
    def update_scanning_animation(self):
        ...
        if not self.paused and self.scheduled_check:
            ...
            if 'Checking' in current_status:
                self.status_label.config(text=f"Checking {self.scanning_chars[self.scanning_index]}")
                    ...
                    if success and result:
                        ...
                        if stock == '1':
                            ...
                            self.product_tree.item(item, values=(
                                ...
                                url,
                                "Best Buy CA" if "bestbuy" in url.lower() else "Unknown",
                            ...
                            self.log_message(f"✅ Found {name} in stock! Pausing monitoring for this item.")
                        else:
                            # Normal status update for not-found items
                            self.update_product_status(item, name or "Unknown Product", url, stock)
                    else:
    ...
    def notify_stock_available(self, product_name: str, stock_count: int, url: str):
        ...
        try:
            ...
            self.log_display.insert('end', f"[{timestamp}] Product found in stock!\n", 'new_message')
    ...
    def update_tab_animation(self):
        ...
        if not self.paused and self.scheduled_check:
            ...
            current_text = self.notebook.tab(self, "text")
            base_text = current_text.split(' ')[-1]  # Get text after any existing indicators
            self.notebook.tab(self, text=f"{self.tab_chars[self.tab_index]} {base_text}")
    ...
    def check_stock(self):
        ...
        try:
            ...
            self.scanning_index = (self.scanning_index + 1) % len(self.scanning_chars)
            self.status_label.config(text=f"Scanning {self.scanning_chars[self.scanning_index]}")
            
            for item in self.product_tree.get_children():
                url = self.product_tree.item(item)['values'][1]  # Get URL from second column
                name = self.product_tree.item(item)['values'][0]  # Get name from first column
    ...
    def add_found_product(self, name: str, url: str):
        ...
        def add_to_list(text_widget):
            if text_widget:
                ...
                text_widget.insert('end', f"{item_number}. [{timestamp}] {short_name} - ", 'normal')
                ...
                text_widget.tag_bind(url_tag, '<Button-1>', lambda e, url=url: self.open_url(url))
                ...
                text_widget.tag_bind(button_tag, '<Button-1>', lambda e, url=url: self.open_url(url))
                text_widget.tag_bind(button_tag, '<Enter>', lambda e: text_widget.config(cursor='hand2'))
                text_widget.tag_bind(button_tag, '<Leave>', lambda e: text_widget.config(cursor=''))
    ...
    def handle_pause_resume(self, item, values):
        ...
        if status == 'Monitoring':
            # Pause monitoring
            self.product_tree.item(item, values=(
                name, url, store, 'Paused', '▶', '🗑'
            ))
            self.log_message(f"Paused monitoring for {name}")
        else:
            # Resume monitoring
            self.product_tree.item(item, values=(
                name, url, store, 'Monitoring', '⏸', '🗑'
            ))
