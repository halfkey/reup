...
class SecurityManager:
    ...
    def secure_file(self, path: Union[str, Path]) -> None:
        ...
        try:
            ...
            os.chmod(path, perms)
            log_security_event("FILE_SECURE", f"Applied secure permissions to {path}")
        except Exception as e:
            log_security_event(
                "FILE_SECURE_ERROR", f"Failed to secure {path}: {str(e)}", "ERROR"
            )
    ...
    def secure_directory(self, path: Union[str, Path]) -> None:
        ...
        try:
            ...
            os.chmod(path, perms)
            log_security_event("DIR_SECURE", f"Applied secure permissions to {path}")
        except Exception as e:
            log_security_event(
                "DIR_SECURE_ERROR", f"Failed to secure {path}: {str(e)}", "ERROR"
            )
