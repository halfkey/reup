...
class BestBuyAPI:
    ...
    def check_stock(self, product_id: str) -> Tuple[bool, str, Dict]:
        ...
        try:
            ...
            log_security_event("API_REQUEST", f"Checking stock for product {product_id}")
            ...
            log_security_event("API_ERROR", f"Timeout checking product {product_id}", "WARNING")
            raise APIError(408, "Request timed out - retrying...")
        except requests.RequestException as e:
            log_security_event("API_ERROR", f"Request failed for product {product_id}: {str(e)}", "ERROR")
            raise APIError(getattr(e.response, 'status_code', 500), str(e))
        except Exception as e:
            log_security_event("API_ERROR", f"Unexpected error for product {product_id}: {str(e)}", "ERROR")
    ...
    def _parse_product_page(self, html: str, product_id: str) -> Tuple[bool, str, Dict]:
        ...
        try:
            ...
            name = (
                page_title or 
                product_data.get('product', {}).get('name') or
                "Unknown Product"
            )
